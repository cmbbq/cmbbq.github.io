...
### 归一化层
归一化可用于抵抗梯度消失。最主要的归一化层是Batch Normalization[Ioffe and Szegedy, 2015][^18]，由超参$D$和可训练参数$\beta_1, \dots,\beta_D$和$\gamma_1, \dots,\gamma_D$组成。给定一批$D$维样本$x_1, \dots, x_B$，先计算每个维度的mean $\widehat{m}_d = \frac{1}{B} \sum_{b=1}^B x_{b,d}$和variance $\widehat{v}_d  = \frac{1}{B} \sum_{b=1}^B (x_{b,d} - \widehat{m}_d)^2$。
